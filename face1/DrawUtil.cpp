...
void DrawUtil::draw_point(Mat &img, int x, int y, int r, int channel) {
    for (int i = x-r; i <= x+r; i++) {
        for (int j = y-r; j <= y+r; j++) {
            if (is_valid_in(img, i, j)) {
...
void DrawUtil::draw_points(Mat &img, vector<Point> &points, int r, int channel) {
    for (auto p : points) {
        draw_point(img, p.x, p.y, r, channel);
    }
}
