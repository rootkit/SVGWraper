...
void BezierUtil::drawCubeBezier(Mat &img, Point p0, Point p1, Point p2, Point p3) {
    int x, y;
    for (float t = 0; t <= 1; t += 0.001) {
        ...
        DrawUtil::drawPoint(img, x, y, 1);
    }
}

void BezierUtil::drawCubeBezier(Mat &img, Point p0, Point p1, Point p2,
                                  Point p3, Scalar color) {
    int x, y;
    for (float t = 0; t <= 1; t += 0.001) {
        x = (int)((1-t)*(1-t)*(1-t)*p0.x + 3*t*(1-t)*(1-t)*p1.x + 3*t*t*(1-t)*p2.x + t*t*t*p3.x);
        y = (int)((1-t)*(1-t)*(1-t)*p0.y + 3*t*(1-t)*(1-t)*p1.y + 3*t*t*(1-t)*p2.y + t*t*t*p3.y);
        DrawUtil::drawPoint(img, x, y, 1, color);
    }
}

void BezierUtil::getControlPoints(Point &p1, Point &thirdOne, Point &thirdTwo,
 ...
 */
vector<Point> BezierUtil::regainNewPoints(vector<Point>& points) {
    vector<Point> newPoints;
    
    for (int i = 0; i < points.size()-1; i+=3) {
        Point p2, p3;
        getControlPoints(points[i], points[i+1], points[i+2],
                           points[i+3], p2, p3);
        if (i == 0) {
...
vector<Point> BezierUtil::getBezier1(vector<Point> &svg) {
...
vector<Point> BezierUtil::getBezier2(vector<Point> &svg) {
    assert((svg.size()-1)%3==0);
 ...
 */
void BezierUtil::getBezier(vector<Point> &svg, vector<Point> &bezier1, vector<Point> &bezier2) {
    bezier1 = getBezier1(svg);
    bezier2 = getBezier2(svg);
}

void BezierUtil::drawBezier(Mat &img, vector<Point> &ctrl) {
    if ((ctrl.size() - 1) % 3 == 0) {
        for (int i = 0; i < ctrl.size()-1; i+=3) {
            drawCubeBezier(img, ctrl[i], ctrl[i+1], ctrl[i+2], ctrl[i+3]);
        }
    } else {
...
void BezierUtil::drawBezier(Mat &img, vector<Point> &ctrl, Scalar color) {
    if ((ctrl.size() - 1) % 3 == 0) {
        for (int i = 0; i < ctrl.size()-1; i+=3) {
            drawCubeBezier(img, ctrl[i], ctrl[i+1], ctrl[i+2], ctrl[i+3], color);
        }
    } else {
        assert(false && "support only quadratic and cube bezier");
    }
...
void BezierUtil::getBezierBorder(vector<Point> &bezier, int &left, int &right,
                                   int &top, int &bottom) {
